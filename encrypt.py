...
def embed_data(image, data_bits):
    ...
    for i in range(len(data_bits)):
        flat[i] = (flat[i] & 0xFE) | data_bits[i]  # Ensure uint8 type
    return flat.reshape(image.shape)

def encrypt():
    img_path = "nature.jpeg"
    if not os.path.exists(img_path):
        messagebox.showerror("Error", "Input image 'nature.jpeg' not found!")
        return
    
    image = cv2.imread(img_path)
    if image is None:
        messagebox.showerror("Error", "Failed to load 'nature.jpeg'.")
        return
